...
const storage = multer.diskStorage({
  destination: "uploads/ministry-order/",
  filename: (req, file, cb) => cb(null, Date.now() + "-" + file.originalname),
});

const upload = multer({ storage });

ministryRouter.post("/add", upload.single("pdf"), async (req, res) => {
  try {
    const conn = await pool.getConnection();
    const title = req.body.title;
    const pdf_url = `/uploads/ministry-order/${req.file.filename}`;
    await conn.query(
      "INSERT INTO ministry_order (title, pdf_url) VALUES (?, ?)",
      [title, pdf_url]
    );
    conn.release();
...
ministryRouter.get("/", async (req, res) => {
  try {
    const conn = await pool.getConnection();
    const data = await conn.query("SELECT * FROM ministry_order");
    res.json(data);
    conn.release();
    //   res.json({ message: "Structure added successfully" });
  } catch (error) {
...
ministryRouter.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await conn.query(
      "SELECT pdf_url FROM ministry_order WHERE id = ?",
      [id]
    );
    ...
    // Delete from DB
    await conn.query("DELETE FROM ministry_order WHERE id = ?", [id]);
    conn.release();
